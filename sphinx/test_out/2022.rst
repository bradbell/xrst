...
10-04
=====
1. The :ref:`heading_links@Labels@Conversion_` of headings was changed
   to not convert colons ``:`` .
2. The example :ref:`preamble.rst` file was chage to use three
...
2. The :ref:`heading_links@Labels@Conversion_` of headings was changed
...
4. There was a problem with the :ref:`code_cmd` when it was inside a list.
   ...
   :ref:`list_example@Code Command` example now displays properly.
...
1. The code command would not include the
   ...
   This has been fixed.
2. An :ref:`list_example@RST Directive` was added to the list example.
...
a heading (becasue Sphinx does not handle this case).
...
#. Change the link to the title for a page from ``@`` *page_name* to
   *page_name* ``-0`` ; see :ref:`heading_links@Labels@Level Zero`
   heading links.
   This has the advantage that the html link uses the page name instead of
   an id number.
#. Do not map characters (except for ``:`` and ``@`` ) when
   automatically creating labels; see :ref:`heading_links@Labels@Conversion_`
...
09-05
=====
The :ref:`xrst_table_of_contents-0` did not build properly unless ``|space|``
...
1. The colon ``:`` was added to the characters that get changed to
   ...
   :ref:`heading_links@Labels@Conversion_` .
...
(this ignores all latex commands); see :ref:`spell_cmd-0`.
