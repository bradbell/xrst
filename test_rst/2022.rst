...
11-28
=====
If :ref:`run_xrst@target` is pdf, the
...
As an example, change ``test_example`` to :ref:`testExample` .
...
2. Add the :ref:`configure_example` page and improve
   the :ref:`get_started` page.
...
11-22
=====
The :ref:`toml_file` was changed to be a sequence of tables.
...
1. No longer necessary to create a sub-directory called ``sphinx``
   in the :ref:`get_started` example.
2. Use ``xrst.toml`` as the default value for :ref:`run_xrst@toml_path` .
3. Change the command link argument flags to use full names; i.e.,
   change --html -> --html_theme and --group -> --group_list .
4. Change the :ref:`suspend_example` to demonstrate documenting
...
11-20
=====
1. Change the root_file command line argument to
   :ref:`run_xrst@toml_path` .
2. Add :ref:`toml_file@project_name` and :ref:`toml_file@root_file`
   to the toml file.
3. Use the notation :ref:`run_xrst@toml_path@project_directory`
...
2. The wish list configuration item was completed using
   ...
   #. The sphinx_dir command line argument was replaced by
      :ref:`toml_file@rst_directory` in the ``toml_file`` file.
...
2. The list example was removed and the :ref:`testExample` was added.
